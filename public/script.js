...
const masVendidos =
JSON.parse(localStorage.getItem(`masVendidos_${restaurantId}`)) || [];

const productos =
[...new Set([...recomendadosManual, ...masVendidos])];

return productos;
}

function renderProductosRecomendados(menu) {
const recomendados = obtenerProductosRecomendados();

if (!recomendados.length) return "";

const productosFiltrados = menu.filter(item =>
recomendados.some(nombre =>
...
const frasesTop = [
"🔥 Uno de los productos más recomendados y pedidos por nuestros clientes.",
"⭐ Favorito por nuestros clientes frecuentes.",
"👑 Producto destacado por su alta demanda en el restaurante.",
"🔥 Elegido constantemente por quienes visitan la casa.",
"⭐ Uno de los platos con mayor aceptación entre nuestros clientes."
];

let html = `
<h3 class="categoria-titulo">🔥 Especialidades de la casa las preparaciones mas apreciadas </h3>
<div class="grid">
`;

productosFiltrados.forEach(item => {
  const fraseRandom =
frasesTop[Math.floor(Math.random() * frasesTop.length)];
html += `
<div class="card recomendado-card">
<div class="cinta-recomendado">
  ⭐ RECOMENDADO
</div>
...
<p>${item.descripcion || "Uno de los productos más recomendados y pedidos por nuestros clientes."}</p>

<p><strong>Precio base:</strong> $${item.precio}</p>
<p><strong>Tiempo estimado base:</strong> ${item.tiempoBase} min</p>

<button onclick="document.getElementById('producto-${item.id}')?.scrollIntoView({ behavior: 'smooth' })">
Ver producto
</button>
</div>
`;
...
contenedor.innerHTML += renderProductosRecomendados(menu);
