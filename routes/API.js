...
router.get("/menu", async (req, res) => {
  try {
    ...
    console.log("Error obteniendo menÃƒÂº:", error);
    res.status(500).json([]);
  }
});
...
router.post("/menu", async (req, res) => {
  ...
  try {
    ...
    // =====================================================
    // GRUK Ã¢â‚¬â€ VALIDACIÃƒâ€œN DE ENTRADA
    // =====================================================
    ...
    if (!Number.isFinite(precioNumerico) || precioNumerico < 0) {
      return res.status(400).json({
        ok: false,
        error: "El precio del producto no es vÃƒÂ¡lido"
      });
    }
    ...
    if (!Number.isFinite(costoNumerico) || costoNumerico < 0) {
      return res.status(400).json({
        ok: false,
        error: "El costo de materia prima no es vÃƒÂ¡lido"
      });
    }
    ...
    if (!Number.isFinite(tiempoNumerico) || tiempoNumerico < 0) {
      return res.status(400).json({
        ok: false,
        error: "El tiempo base no es vÃƒÂ¡lido"
      });
    }
    ...
    // =====================================================
    // GRUK Ã¢â‚¬â€ UNIDAD ATÃƒâ€œMICA CORE + RESTAURANTE
    // =====================================================
    ...
    await session.withTransaction(async () => {
      ...
      // ===================================================
      // 3. CREAR IDENTIDAD ECONÃƒâ€œMICA EN GRUK CORE
      // ===================================================
      ...
      if (!productoCore) {
        throw new Error(
          "No fue posible crear la identidad econÃƒÂ³mica del producto"
        );
      }
      // ===================================================
      // 4. CREAR REPRESENTACIÃƒâ€œN EN GRUK RESTAURANTES
      // ===================================================
    ...
    // =====================================================
    // GRUK REALTIME
    // Solo despuÃƒÂ©s de COMMIT exitoso.
    // =====================================================
...
router.put("/menu/:id", async (req, res) => {
  ...
  try {
    ...
    // =====================================================
    // GRUK Ã¢â‚¬â€ VALIDACIÃƒâ€œN ESTRICTA DE ACTUALIZACIÃƒâ€œN
    // =====================================================

    if (!Number.isInteger(id) || id <= 0) {
      return res.status(400).json({
        ok: false,
        error: "Identificador de producto no vÃƒÂ¡lido"
      });
    }
    ...
    // =====================================================
    // GRUK Ã¢â‚¬â€ UNIDAD ATÃƒâ€œMICA DE ACTUALIZACIÃƒâ€œN
    ...
    await session.withTransaction(async () => {
      ...
      // ===================================================
      // 3. RESOLVER IDENTIDAD ECONÃƒâ€œMICA CORE
      // ===================================================
      ...
      if (productoActual.productoServicioId) {
        ...
        // ===============================================
        // MIGRACIÃƒâ€œN TRANSACCIONAL DE PRODUCTO LEGACY
        // ===============================================
      ...
      // ===================================================
      // 4. ACTUALIZAR IDENTIDAD ECONÃƒâ€œMICA CORE
      // ===================================================
      ...
      // ===================================================
      // 5. ACTUALIZAR REPRESENTACIÃƒâ€œN RESTAURANTE
      // ===================================================
...
router.put("/menu/:id/stock", async (req, res) => {
  try {
    ...
    // =====================================================
    // GRUK RESTAURANTES Ã¢â‚¬â€ DISPONIBILIDAD OPERACIONAL
    // =====================================================
    ...
    if (
      ...
      return res.status(400).json({
        ok: false,
        error: "Estado de disponibilidad no vÃƒÂ¡lido"
      });
    }
...
router.delete("/menu/:id", async (req, res) => {
  try {
    ...
    // =====================================================
    // GRUK Ã¢â‚¬â€ VALIDACIÃƒâ€œN DE IDENTIDAD
    // =====================================================
    ...
    // =====================================================
    // GRUK RESTAURANTES Ã¢â‚¬â€ LOCALIZAR PRODUCTO
    // =====================================================
    ...
    // =====================================================
    // GRUK CORE Ã¢â‚¬â€ VERIFICAR PROPIEDAD EMPRESARIAL
    // =====================================================
    ...
    if (!restaurante.empresaId) {
      return res.status(409).json({
        ok: false,
        error: "El restaurante todavÃƒÂ­a no estÃƒÂ¡ vinculado a una empresa"
      });
    }
    ...
    // =====================================================
    // GRUK CORE Ã¢â‚¬â€ RETIRO LÃƒâ€œGICO
    // =====================================================
    //
    // La identidad econÃƒÂ³mica NO se elimina.
    // Debe permanecer disponible para:
    //
    // - ventas histÃƒÂ³ricas
    // - costos histÃƒÂ³ricos
    // - reportes
    // - auditorÃƒÂ­a
    // - GRUK Intelligence
    // - memoria empresarial
    ...
    // =====================================================
    // GRUK RESTAURANTES Ã¢â‚¬â€ RETIRAR DE LA VERTICAL
    // =====================================================
    ...
    try {
      ...
      // CompensaciÃƒÂ³n provisional:
      // si falla la eliminaciÃƒÂ³n del Menu despuÃƒÂ©s de retirar CORE,
      ...
      if (productoCore) {
        try {
          ...
          console.error(
            "CRITICAL: fallo restaurando ProductoServicio despuÃƒÂ©s de DELETE:",
            rollbackError
          );
        }
      }
    ...
    // =====================================================
    // GRUK REALTIME Ã¢â‚¬â€ NOTIFICAR A LA VERTICAL
    // =====================================================
    ...
    console.error(
      "Error retirando producto del menÃƒÂº:",
      error
    );
...
router.post("/pedido", async (req, res) => {
  try {
    ...
    // =====================================================
    // GRUK RESTAURANTES Ã¢â‚¬â€ VALIDACIÃƒâ€œN OPERACIONAL
    // =====================================================
    ...
    if (
      ...
      return res.status(400).json({
        ok: false,
        error: "MÃƒÂ©todo de pago no vÃƒÂ¡lido"
      });
    }

    // =====================================================
    // GRUK SECURITY Ã¢â‚¬â€ IDENTIDAD DEL PRODUCTO
    ...
    if (
      ...
      return res.status(400).json({
        ok: false,
        error: "Identidad de producto no vÃƒÂ¡lida"
      });
    }
    ...
    if (!menuItem.productoServicioId) {
      return res.status(409).json({
        ok: false,
        error:
          "El producto todavÃƒÂ­a no tiene identidad GRUK CORE"
      });
    }

    // Si el cliente envÃƒÂ­a productoServicioId,
    ...
    if (
      ...
      return res.status(409).json({
        ok: false,
        error:
          "La identidad econÃƒÂ³mica enviada no corresponde al producto"
      });
    }
    ...
    // =====================================================
    // GRUK RESTAURANTES Ã¢â‚¬â€ VALIDAR EXTRA
    ...
    if (
      ...
      if (
        ...
        return res.status(409).json({
          ok: false,
          error:
            "El extra tiene una configuraciÃƒÂ³n econÃƒÂ³mica invÃƒÂ¡lida"
        });
      }
    ...
    // =====================================================
    // GRUK CORE Ã¢â‚¬â€ AUTORIDAD ECONÃƒâ€œMICA
    ...
    if (
      ...
      return res.status(409).json({
        ok: false,
        error:
          "El producto tiene un precio invÃƒÂ¡lido"
      });
    }
    ...
    // =====================================================
    // GRUK Ã¢â‚¬â€ CREACIÃƒâ€œN TRAZABLE DEL PEDIDO
    // Vertical Restaurante Ã¢â€ â€ Identidad econÃƒÂ³mica CORE
    // =====================================================
    ...
    // =====================================================
    // GRUK REALTIME
    // Solo despuÃƒÂ©s de persistencia confirmada.
    // =====================================================
...
router.get("/pedidos", async (req, res) => {
  try {
    ...
    // Ã°Å¸â€˜â€¡ SOLO si viene sedeId, filtra por sede
    if (sedeId) {
...
router.post("/llamar-mesero", async (req, res) => {
  try {
    ...
    if (mesero.estado === "ocupado") {
      return res.status(400).json({
        ok: false,
        error: "Ese mesero ya estÃƒÂ¡ ocupado"
      });
    }
    ...
    const llamado = new Llamado({
      restaurantId,
      mesa: Number(mesa),
      mensaje: mensaje || "Mesa necesita atenciÃƒÂ³n",
      estado: "pendiente",
    ...
    llamados.forEach(llamado => {
      if (!mesasMap[llamado.mesa]) {
        mesasMap[llamado.mesa] = {
          ...
          mensaje: llamado.mensaje || "Mesa necesita atenciÃƒÂ³n",
...
router.post("/admin/login", async (req, res) => {
  try {
    ...
    if (!usuario || !password) {
      return res.status(400).json({
        ok: false,
        error: "Faltan usuario o contraseÃƒÂ±a"
      });
    }
...
router.post("/restaurante/registro", async (req, res) => {
  try {
    ...
    if (existeCorreo) {
      return res.status(400).json({
        ok: false,
        error: "Ese correo ya estÃƒÂ¡ registrado"
      });
    }
...
// Consulta estado de suscripciÃƒÂ³n
router.get("/restaurante/estado-suscripcion", async (req, res) => {
  try {
    ...
    console.log("Error consultando suscripciÃƒÂ³n:", error);
    res.status(500).json({
      ok: false,
      error: "Error interno consultando suscripciÃƒÂ³n"
    });
  }
});
