...
function construirDecisionFingerprint({
 ...
 const payload={
  agenda:agenda?{
   ...
   saldoOperativo:agenda.saldoOperativo??null,
   reservasActivasTotal:Number(agenda.reservasActivas?.total||0),
   reservaUtilidadDueno:Number(agenda.reservasActivas?.utilidadDueno||0),
   ...
   politica:{
    activa:Boolean(agenda.politicaPriorizacionPagos?.usar_precedencia_categoria),
    categorias:[...(agenda.politicaPriorizacionPagos?.precedencia_categorias||[])]
   },
   excepciones:(agenda.excepcionesPrioridadPago||[]).map(item=>({
    id:String(item._id||""),
    origenTipo:item.origenTipo,
    origenId:String(item.origenId||""),
    motivo:item.motivo,
...
async function tomarDecision(empresaId,opciones={}){
 ...
 for(const r of reportes){
  ...
  for(const h of r.hallazgos||[]){
   ...
   if(regla)candidatos.push({reporte:r,hallazgo:h,regla});
  }
 }
 ...
 const riesgoAgenda=agenda?.estado7d==="DEFICIT_AUN_COBRANDO_TODO"
  ...
  : agenda?.estado7d==="DEPENDE_DE_COBROS"
    ...
    : null;
 const nuevaDecision=(await Decision.create({empresaId:new mongoose.Types.ObjectId(String(empresaId)),sedeId:null,decisionFingerprint,decision_general:{situacion:construirSituacion(criticos,ordenes),causa_raiz:causaAgenda||(principal?principal.hallazgo.evidencia:"No hay hallazgos accionables con los datos actuales."),prediccion:agenda?.requiereDecision?"Sin resolver la señal financiera antes de la fecha critica, la cobertura operativa de corto plazo puede deteriorarse.":(principal?"Sin correccion, el KPI asociado puede continuar fuera del objetivo.":"Mantener seguimiento de los cinco KPI principales.")},ordenes_por_departamento:ordenes,contexto_financiero:agenda?{fuente:agenda.fuente,estado7d:agenda.estado7d,confiabilidad:agenda.confiabilidad,saldoActual:agenda.saldoActual,saldoOperativo:agenda.saldoOperativo,reservasActivasTotal:Number(agenda.reservasActivas?.total||0),reservaUtilidadDueno:Number(agenda.reservasActivas?.utilidadDueno||0),obligaciones7d:agenda.obligaciones7d,cobros7d:agenda.cobros7d,faltanteConCajaActual:agenda.faltanteConCajaActual,faltanteAunCobrandoTodo:agenda.faltanteAunCobrandoTodo,montoCobrosPriorizados:agenda.montoCobrosPriorizados||0,faltanteDespuesCobrosPriorizados:agenda.faltanteDespuesCobrosPriorizados||0,cobrosPriorizados:(agenda.cobrosPriorizados||[]).map(item=>({ventaId:item.id||null,descripcion:item.descripcion||"",monto:Number(item.monto||0),fechaVencimiento:item.fechaVencimiento||null,clasificacion:item.clasificacion})),politicaPriorizacionPagos:agenda.politicaPriorizacionPagos?{usar_precedencia_categoria:Boolean(agenda.politicaPriorizacionPagos.usar_precedencia_categoria),precedencia_categorias:[...(agenda.politicaPriorizacionPagos.precedencia_categorias||[])],updatedAt:agenda.politicaPriorizacionPagos.updatedAt||null,updatedBy:agenda.politicaPriorizacionPagos.updatedBy||null}:null,excepcionesPrioridadPago:(agenda.excepcionesPrioridadPago||[]).map(item=>({excepcionId:item._id||null,origenTipo:item.origenTipo,origenId:item.origenId,motivo:item.motivo,expiresAt:item.expiresAt,createdBy:item.createdBy||null})),obligacionesPriorizadas:(agenda.obligacionesPriorizadas||[]).map(item=>({origenId:item.id||null,tipo:item.tipo||null,descripcion:item.descripcion||"",categoria:item.categoria||null,tercero:item.tercero||"",monto:Number(item.monto||0),fechaVencimiento:item.fechaVencimiento||null,estadoCobertura:null})),planPagosCajaActual:(agenda.planPagosCajaActual||[]).map(item=>({origenId:item.id||null,tipo:item.tipo||null,descripcion:item.descripcion||"",categoria:item.categoria||null,tercero:item.tercero||"",monto:Number(item.monto||0),fechaVencimiento:item.fechaVencimiento||null,estadoCobertura:item.estadoCobertura})),planPagosConCobros:(agenda.planPagosConCobros||[]).map(item=>({origenId:item.id||null,tipo:item.tipo||null,descripcion:item.descripcion||"",categoria:item.categoria||null,tercero:item.tercero||"",monto:Number(item.monto||0),fechaVencimiento:item.fechaVencimiento||null,estadoCobertura:item.estadoCobertura})),fechaCritica:agenda.fechaCritica}:undefined,confianza_global:Math.max(0,Math.min(100,confianza)),riesgo_si_no_se_hace:riesgoAgenda||(principal?principal.hallazgo.evidencia:"No se identifico riesgo cuantificado."),como_medir_exito_en_7_dias:agenda?.requiereDecision?"Recalcular Tesoreria y verificar que la brecha de caja de 7 dias sea cero o que la cobertura quede demostrada con datos completos.":(ordenes.length?"Recalcular los KPI de las ordenes y comparar contra sus objetivos.":"Generar nuevamente los cinco reportes y verificar su estado."),reportesOrigen:reportes.map(r=>r._id),createdBy:null,deletedAt:null})).toObject();
